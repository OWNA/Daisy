...
# Load scaling params from JSON file
scaling_path = "trading_bot_data/lgbm_model_BTC_USDTUSDT_l2_only_scaling.json"
try:
    with open(scaling_path, 'r') as f:
        scaling_params = json.load(f)
    target_mean = scaling_params['target_mean']
    target_std = scaling_params['target_std']
    print(f"Loaded scaling from {scaling_path}: mean={target_mean}, std={target_std}")
    predictor.set_scaling_params(target_mean, target_std)
except FileNotFoundError:
    print(f"Warning: Scaling file {scaling_path} not found, using fallback")
    predictor.set_scaling_params(0.0, 1.0)
