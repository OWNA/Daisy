...
class StrategyBacktester:
    ...
    def _adapt_l2_to_ohlcv(self, df_input):
        """
        Adapts L2-only data to OHLCV format for backtesting.
        Maps L2 columns to traditional OHLCV columns.
        """
        df = df_input.copy()
        ...
        print("Info (Backtester): Adapting L2-only data to OHLCV format.")
        
        # Map L2 columns to OHLCV equivalents
        if 'weighted_mid_price' in df.columns:
            df['close'] = df['weighted_mid_price']
        elif 'microprice' in df.columns:
            df['close'] = df['microprice']
        else:
            print("Error (Backtester): No suitable price column found in L2 data.")
            return pd.DataFrame()
        
        # Approximate high/low from bid/ask or close price
        if 'ask_price_1' in df.columns and 'bid_price_1' in df.columns:
            df['high'] = df['ask_price_1']
            df['low'] = df['bid_price_1']
        else:
            # Fallback: use close price with small spread approximation
            spread_pct = 0.0005  # 0.05% spread approximation
            df['high'] = df['close'] * (1 + spread_pct)
            df['low'] = df['close'] * (1 - spread_pct)
        
        # Use L2 volatility instead of ATR
        if 'l2_volatility_1min' in df.columns:
            df['atr'] = df['l2_volatility_1min']
        elif 'realized_volatility' in df.columns:
            df['atr'] = df['realized_volatility']
        else:
            # Fallback ATR calculation from close price
            df['atr'] = df['close'] * 0.02  # 2% fallback
    ...
    def run_backtest(self, df_with_signals):
        ...
        # First adapt L2-only data to OHLCV format if needed
        adapted_df = self._adapt_l2_to_ohlcv(df_with_signals.copy())
        if adapted_df.empty:
            print("Error (Backtester): L2 data adaptation failed.")
            return None, None

        if 'signal' not in adapted_df.columns or 'close' not in adapted_df.columns:
            print("Error (Backtester): DataFrame must contain 'signal' and 'close' columns.")
            return None, None

        backtest_df = self._ensure_atr(adapted_df)
        ...
        final_results_df['equity'] = final_results_df['equity'].ffill()

        first_valid_equity_idx = final_results_df['equity'].first_valid_index()
        if first_valid_equity_idx is not None:
            final_results_df.loc[:first_valid_equity_idx-1, 'equity'] = self.initial_balance
