    ...
    def plot_feature_importance(
        ...
        if use_shap:
            ...
            try:
                ...
                if 'target' in df_shap_data_full.columns:
                    df_shap_data = df_shap_data_full[trained_features].copy()
                else:
                    # If no target column, just use the features
                    available_features = [f for f in trained_features if f in df_shap_data_full.columns]
                    if not available_features:
                        print(
                            "Error (Visualizer): No trained features found in prepared data. "
                            "Falling back."
                        )
                        self.plot_feature_importance(
                            model_booster,
                        ...
                        return
                    df_shap_data = df_shap_data_full[available_features].copy()
        ...
        try:
            ...
            if hasattr(model_booster, 'feature_importances_'):
                ...
                # Try the method version (for LightGBM booster objects)
                try:
                    ...
                    print(
                        f"Error (Visualizer): Could not get feature importance: {e}"
                    )
                    return
            else:
                print(
