    ...
    def predict_signals(
        ...
        try:
            if use_ensemble:
                ...
                # Use unscaled predictions for signal generation if available
                if "pred_unscaled_target" in result_df and not result_df["pred_unscaled_target"].isna().all():
                    # Use a price-based threshold (e.g., $5 movement)
                    price_threshold = _threshold * 500  # Scale threshold to price terms
                    result_df["signal"] = np.select(
                        [result_df["pred_unscaled_target"] > price_threshold, 
                         result_df["pred_unscaled_target"] < -price_threshold],
                        [1, -1],
                        default=0
                    )
